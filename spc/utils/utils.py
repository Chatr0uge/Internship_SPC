...
def window_around_max(pulse, window_size = 1000) : 
...
def convert_to_database(dir : str, db_path : str) -> None :
    ...
    for i in range(len(params)): 
        IQ_list = []
        IQ_0 = np.load(dir + f'/id_{i}/IQ0.npy')
        for file in os.listdir(dir + f'/id_{i}') : 
            if file[:3] == 'IQ_':
                
                IQ = np.load(dir + f'/id_{i}/{file}')
                a, b = IQ[0], IQ[1]
        ...
        pulse_list.append(np.array(abs(pl)))
        delay_list.append(np.array([delay(*IQ) for IQ in IQ_list]))
        pulse_width.append(np.array([delay_dispersion(*IQ) for IQ in IQ_list]))
    np.save('pulse_list.npy', pulse_list, allow_pickle = True)
    np.save('delay_list.npy', delay_list, allow_pickle = True)
    np.save('pulse_width.npy', pulse_width, allow_pickle = True)
    ...
    for i , sim_params in tqdm(enumerate(params)) : 
        ...
        d2 = {'id': i, 'amplitude': sim_params['amp'], 'lcx': sim_params['lcx'], 'lcy': sim_params['lcy'], 'bm': sim_params['bm'],
                'dt': 4.0849673202614387e-13, 'pulse_list': np.mean(pulse_list[i], axis = 0), 
                'delay_list': delay_list[i], 'pulse_width': np.array(pulse_width[i]), 'skewness': ds['skewness'], 
